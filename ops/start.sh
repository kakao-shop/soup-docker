...
# elasticdump를 이용해서 json 파일에 저장된 데이터를 elasticsearch에 import
...
# 4. spring boot에서 alias를 통해 인덱스에 접근하므로 위에서 import한 상품 정보 index에 alias를 설정
...
# 5. docker-compose 명령을 통해 backend, frontend 컨테이너를 실행
